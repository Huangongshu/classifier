...
train = boston.data #获取特征数据
target = boston.target #获取标签数据
...
def padding(x):
    ...
    for line in x:
        ...
        for l in line:
            tempList.extend([l,l])
        X.append([np.array(tempList),np.array(tempList)])
    return X

X_train_Double = padding(X_train)
X_train_Double = np.array(X_train_Double)
X_test_Double = padding(x_test)
...
class Regression_Training():
    ...
    def __init__(self,net,learning_rate = [1e-3,1e-5,1e-7],
                 batch_size = 1024, epoch = 2000,
                 use_more_gpu = False, weight_decay=1e-8,
                 device=0 ,save_path='CNN_Result'):
        self.net = net
...
model = TransAm(feature_size=26,num_layers=4,dropout=0.5)
grt = Regression_Training(model,learning_rate = [1e-3,1e-6,1e-8],batch_size = 512,use_more_gpu = False,weight_decay=1e-3, device=0 ,save_path='transformer_Result',epoch = 20000)
